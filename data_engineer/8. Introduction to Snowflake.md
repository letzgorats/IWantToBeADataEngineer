...
## Snowflake에서 반정형 데이터(semi-structured 데이터) 다루기
- 구조화된 데이터(Structured) vs 반정형 데이터(Semi-structured) 차이점
- Snowflake 에서 `JSON` 을 저장하는 방법
- `JSON` 을 Snowflake 에서 변환하고 조회하는 방법(PARSE_JSON, OBJECT_CONSTRUCT, JSON 쿼리)
- 중첩 JSON(Nested JSON) 데이터 처리 방법

## 구조화된 데이터 vs 반정형 데이터

### 구조화된 데이터(Structured Data)
- 고정된 스키마(schema)가 있는 정형 데이터
(ex) 고객 데이터(고객 ID,이름,나이,이메일 등)

### 반정형 데이터(Unstructured Data)
- 고정된 스키마 없이 유연한 데이터 저장 가능
(ex) 고객마다 보유한 이메일 개수가 다를 수 있다.(일부 고객은 이메일이 2개, 일부는 1개)
- 대표적인 반정형 데이터 형식 : JSON, XML, Avro, Parquet 등
(ex)
```JSON
{
  "cust_id": 1,
...
```
- 고객마다 이메일 개수가 다를 수 있다.(**배열 사용 가능** )
- 고객 주소(address)처럼 중첩된 데이터 저장이 가능하다.(**Nested JSON** )

## Snowflake 에서 JSON 을 저장하는 방법 : VARIANT
- Snowflake는 `JSON` 을 저장하기 위해 `VARIANT` 데이터 타입을 사용한다.(PostgreSQL은 `JSONB`를 사용한다.)
- **`VARIANT`** 는 **`JSON`, `XML` 같은 반정형 데이터를 저장할 수 있는 데이터 타입이다.**
- **객체 데이터, 배열 데이터 모두 지원한다.**
- 객체 데이터 : {"key" : "value")
- 배열 데이터 : ["list", "of", "values"]
...
``` 
- `VARIANT` 컬럼을 사용하면 JSON 데이터를 유연하게 저장할 수 있다!

## 반정형 데이터 다루기 : JSON 변환 및 조회
- Snowflake는 `JSON` 데이터를 다룰 수 있는 여러 가지 내장 함수를 제공한다.

### (1) PARSE_JSON 함수
- **JSON 문자열** 을 Snowflake의 **VARIANT 타입으로 변환**
```SQL
SELECT PARSE_JSON('{"cust_id": 1, "name": "John Doe"}');
```
- 결과
```JSON
{
...
### (2) OBJECT_CONSTRUCT 함수
- **여러 키-값을 받아서 JSON 객체로 반환**
```SQL
SELECT OBJECT_CONSTRUCT('cust_id', 1, 'name', 'John Doe');
-- 여기서 키와 각각의 값을 인수로 전달하는데, 콜론이 아닌 쉼표로 구분하여 전달한다.
```
- 결과
```JSON
{
...
## JSON 데이터 조회하기
- Snowflake는 JSON 데이터를 다룰 때, **콜론(:)** 과 **점(.)** 표기법을 제공한다.

### (1) JSON 키 값 조회
- 기본 JSON 데이터 조회(콜론 표기법 사용)
```SQL
SELECT customer_info:age, customer_info:name -- using colon to access data from column 
FROM customers;
```
- 결과
```pgSQL
customer_info:age   | customer_info:name
--------------------+--------------------
30                  | John Doe
...
- 중첩된 데이터 조회(ex. 주소 정보)
```SQL
SELECT customer_info:address:city, customer_info:address:state
FROM customers;
```
- 결과
```pgSQL
...
New York   | NY
```
-> **중첩 JSON 키는 콜론(:) 을 사용하여 접근** 할 수 있다! 

### (3) 점(.) 표기법을 활용한 JSON 조회
- 점(.)을 사용하여 키에 접근 가능(**첫 번째 키는 콜론, 이후는 점 사용** )
```SQL
SELECT customer_info:address.city, customer_info:address.state
FROM customers;
```
...
New York   | NY
```
-> **콜론(:) 과 점(.) 표기법 모두 사용 가능** 하다!

## JSON 배열 데이터 조회하기
- `JSON 배열 데이터` 를 Snowflake에서 조회하는 방법을 알아보자.

### (1) JSON 배열 값 조회
(ex) emails 배열에서 첫 번째 값 가져오기
```SQL
SELECT customer_info:emails[0] AS primary_email
FROM customer
```
- 결과
```
...
john@example.com
```
-> 배열은 **키[인덱스]** 형식으로 접근 할 수 있다!

### (2) JSON 배열을 테이블 형태로 변환하기 : FLATTEN 함수
(ex) 배열 데이터를 여러 행으로 변환 - FLATTEN 함수 사용
```SQL
SELECT value as email
FROM customers, LATERAL FLATTEN(input => customer_info:emails);
```
- 결과
```MARKDOWN
email
...
doe@example.com
```
-> **FLATTEN 함수로 JSON 배열을 테이블 형태로 변환** 할 수 있다!

## 정리

### 1. VARIANT 데이터 타입을 활용하여 JSON 을 저장할 수 있다.
### 2. PARSE_JSON,OBJECT_CONSTRUCT 등의 내장 함수를 이용해 JSON 변환이 가능하다.
### 3. 콜론(:) 및 점(.) 표기법을 사용하여 JSON 데이터를 쉽게 조회할 수 있다.
### 4. FLATTEN 함수를 사용하여 JSON 배열을 테이블 형태로 변환할 수 있다.

...
| 함수           | 입력값                     | 사용 상황            |
| ------------ | ----------------------- | ---------------- |
| `json.load`  | 파일 객체 (`open()`으로 연 파일) | 파일에서 바로 JSON 읽기  |
| `json.loads` | 문자열 (`str`)             | 문자열에 저장된 JSON 파싱 |
...
with open('data.json', 'r', encoding='utf-8') as f:
    data = json.load(f)   # 파일 객체 f를 바로 넣음
...
data = json.loads(json_str)  # 문자열 json_str을 파싱
print(type(data))  # dict

```

> **API 응답 처리: requests 라이브러리를 쓸 때는 response.json()이 내부적으로 json.loads(response.text)를 실행하므로**, 직접 loads 호출할 필요가 없습니다.(API 호출해서 할 때는, 그냥 `.json()`을 하면 된다.)
...
이번 장에서는 지금까지 배운 API 활용 능력을 확장하여 Twitter Streaming API를 깊이 있게 다룹니다.
실시간으로 트윗을 스트리밍하고, 키워드 필터링, 기본 분석, 시각화까지 연결하는 과정을 배웁니다.

### 1. The twitter API and Authentication

#### 📌 특징
- Twitter API는 인증이 필요합니다.
- 접근하려면:
  1) Twitter 계정 생성
  2) [Twitter Developer Platform](https://developer.x.com/en) → App 생성
  3) `API Key`, `API Secret`, `Access Token`, `Access Token Secret` 발급
- 이 4가지 값이 OAuth 인증에 사용됩니다.

### 2. Twitter API 종류
| API 종류            | 설명                       |
| ----------------- | ------------------------ |
| **REST API**      | 트위터 데이터 읽기/쓰기            |
| **Streaming API** | 실시간 트윗 모니터링              |
| **Public Stream** | 모든 공개 데이터 중 일부 샘플        |
| **Firehose API**  | 모든 공개 트윗 전체 (유료, 제한적 접근) |

> 이번 장에서는 **Streaming API**의 `GET statuses/sample` 엔드포인트를 사용합니다.
...
### 4. Tweepy 로 트윗 스트리밍하기
`tweepy` 패키지를 사용하면 Python 에서 Twitter API 를 간편하게 다룰 수 있습니다.
...
### 5. 실시간 데이터 활용
- 키워드 필터링: 원하는 주제만 수집 가능 (track 파라미터)
- 분석 예시:
  - 특정 단어 빈도 계산
  - 언어별 트윗 비율
  - 작성 시간대별 분포
- 시각화: matplotlib, seaborn, wordcloud 등 활용 가능

### 📌  정리
| 개념            | 설명                       |
| ------------- | ------------------------ |
| OAuth 인증      | API 키/시크릿, 액세스 토큰/시크릿 필요 |
| Streaming API | 실시간 트윗 수집 가능             |
| Tweepy        | 파이썬 트위터 API 라이브러리        |
| track 필터      | 키워드 기반 트윗 필터링            |
| 데이터 형식        | JSON(dict)로 파싱 가능        |


### 마무리
이번 장을 통해 **실시간 데이터 스트리밍**과 **API 인증 절차** 를 익혔습니다. 
이로써, '`로컬파일(텍스트,Excel,SAS,Stata,MATLAB,Pickle,HDF5) 불러오기`, `SQL로 관계형 DB에서 데이터 가져오기`, `웹에서 파일/HTML 가져오기 + 웹 스크래핑`, `API와 JSON 데이터 파싱`, `실시간 데이터 스트리밍과 분석`' 을 모두 살펴봤습니다.
